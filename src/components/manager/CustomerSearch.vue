<template>
  <div>
    <ul v-bind:key="data" v-for="data in datas">
      <li>
        <div>
          <img style="float: left;width: 100px; height: 100px;" v-bind:src="data.url"/>
          <div style="float: left; padding-left: 30px">
            <h3 style="display: block; margin: 20px;">{{ data.name }}</h3>
            <p style="display: block; margin: 20px;">个性签名：{{ data.description }}</p>
          </div>

          <button style="float: right;">Delete</button>
          <button style="float: right">Blacked</button>
      ...
      datas: [
        {
          ...
          name: '007号用户',
          description: '我是007，我是一个chinese 用户'
        },
        {
